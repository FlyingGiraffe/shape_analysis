...
if isAxes
    if size(f,1) == mesh.numVertices
        msh = patch('vertices',X,'Faces',T,'FaceColor','interp','CData',double(f),'edgecolor','none','parent',h); 
    else
        msh = patch('vertices',X,'Faces',T,'FaceColor','flat','CData',double(f),'edgecolor','none','parent',h); 
    end
    axis(h,'equal');
    colorbar('peer',h);
    ...
    figure(h);
    if size(f,1) == mesh.numVertices
        msh = patch('vertices',X,'Faces',T,'FaceColor','interp','CData',double(f),'edgecolor','none'); 
    else
        msh = patch('vertices',X,'Faces',T,'FaceColor','flat','CData',double(f),'edgecolor','none'); 
    end
    axis equal;
    colorbar;
