...
color           = getoptions(options, 'color', [.7 .7 .7]);
...
if ~isempty(color)
    h = patch('vertices',vertex,'faces',face,'FaceVertexCData',ones(size(vertex,1),1)*color, 'FaceColor','interp');
elseif isempty(face_vertex_color)
